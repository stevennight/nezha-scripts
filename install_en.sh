...
install_agent() {
    ...
    if [ -z "$_version" ]; then
        err "Fail to obtain Agent version, please check if the network can link https://api.github.com/repos/stevennight/nezha-agent/releases/latest"
        return 1
    else
...
restart_and_update_standalone() {
    ...
    if [ -z "$_version" ]; then
        err "Fail to obtain Dashboard version, please check if the network can link https://api.github.com/repos/stevennight/nezha/releases/latest"
        return 1
    else
        echo "The current latest version is: ${_version}"
    fi
