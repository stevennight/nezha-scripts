...
install_agent() {
    ...
    if [ -z "$_version" ]; then
        err "获取 Agent 版本号失败，请检查本机能否链接 https://api.github.com/repos/stevennight/nezha-agent/releases/latest"
        return 1
    else
...
restart_and_update_standalone() {
    ...
    if [ -z "$_version" ]; then
        err "获取 Dashboard 版本号失败，请检查本机能否链接 https://api.github.com/repos/stevennight/nezha/releases/latest"
        return 1
    else
        echo "当前最新版本为： ${_version}"
    fi
